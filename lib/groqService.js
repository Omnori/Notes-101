...
async function summarizeTranscriptWithGroq(transcriptText, customApiKey = null, customModel = null, guildId = null) {
    const apiKey = customApiKey || process.env.GROQ_API_KEY;
    if (!apiKey) {
        throw new Error('Groq API key is not configured for this server.');
    }
    ...
    const estimatedTokens = Math.ceil(transcriptText.length / 4) + 600;
