...
/**
 * Fetches fresh Org Info page content and returns plain text for context injection.
 * Non-fatal: returns an empty string on any failure or missing configuration.
 */
async function fetchOrgInfoContext(token, orgInfoPageId) {
    if (!token || !orgInfoPageId) return '';
    try {
...
Your task is to review tonight's meeting notes and compare them with the current Org Info page content.
...
1. Output a STRICT JSON array of objects with keys: "action", "section", "content".
   [
     {
       "action": "add" | "update",
       "section": "People & Roles" | "Active Projects" | "Decisions & Policies",
...
3. "section": Must strictly match one of the sections on the page: "People & Roles", "Active Projects", "Decisions & Policies" (or an appropriate specific section name).
...
const CANONICAL_SECTIONS = ['People & Roles', 'Active Projects', 'Decisions & Policies'];
...
function normalizeSection(rawSection) {
    ...
    if (sanitized.includes('people') || sanitized.includes('role') || sanitized.includes('team') || sanitized.includes('member')) {
        return 'People & Roles';
    }
    if (sanitized.includes('project') || sanitized.includes('deliverable') || sanitized.includes('initiative') || sanitized.includes('product')) {
        return 'Active Projects';
    }
    if (sanitized.includes('decision') || sanitized.includes('policy') || sanitized.includes('rule') || sanitized.includes('guideline')) {
